...
function snowplow_track() {
	if(wp_script_is('sp-init')){
		...
		if(is_singular('post')){	
			...
			$content_raw = $this_post->post_content;
			$content_filtered = apply_filters('the_content', $content_raw);
			$content_text = wp_strip_all_tags($content_raw, true);
			$post_links = preg_match_all('/<a\s[^>]*href=[^>]*>/siU', $content_raw);
			$post_headings = preg_match_all('/<h\d[^>]*\>/siU', $content_raw);
			$post_images = preg_match_all('/<img[^>]*\>/siU', $content_raw);
			$post_videos = preg_match_all('/<(video|embed|iframe)[^>]*\>/siU', $content_raw);
